...
EditTodoDialog::EditTodoDialog(const Todo& existingTodo, TodoDatabase* database, QWidget *parent)
    : QDialog(parent), todo(existingTodo) {

    setWindowTitle("Edit Todo");
    setMinimumWidth(500);
    setStyleSheet(R"(
        QDialog {
    ...
    titleInput->setText(QString::fromStdString(existingTodo.getTitle()));
    ...
    descriptionInput->setPlainText(QString::fromStdString(existingTodo.getDescription()));
    formLayout->addRow("Notes", descriptionInput);

    // Category (editable dropdown)
    categoryInput = new QComboBox(this);
    categoryInput->setEditable(true);
    categoryInput->lineEdit()->setPlaceholderText("Optional");
    ...
    categoryInput->setCurrentText(QString::fromStdString(existingTodo.getCategory()));
    ...
    priorityCombo->setCurrentIndex(existingTodo.getPriority() - 1);
    ...
    if (existingTodo.getDueDate().has_value()) {
        hasDueDateCheckbox->setChecked(true);
        QDateTime dt = QDateTime::fromSecsSinceEpoch(existingTodo.getDueDate().value());
        dueDateInput->setDate(dt.date());
        dueDateInput->setEnabled(true);
    } else {
        hasDueDateCheckbox->setChecked(false);
    ...
    // Connect signals
    connect(saveButton, &QPushButton::clicked, this, &EditTodoDialog::onSave);
    connect(cancelButton, &QPushButton::clicked, this, &EditTodoDialog::onCancel);
    connect(hasDueDateCheckbox, &QCheckBox::toggled, this, &EditTodoDialog::onDueDateToggled);
...
void EditTodoDialog::onDueDateToggled(bool checked) {
    dueDateInput->setEnabled(checked);
}

void EditTodoDialog::onSave() {
    ...
    todo.setTitle(title.toStdString());
    todo.setDescription(descriptionInput->toPlainText().toStdString());
    todo.setCategory(categoryInput->currentText().trimmed().toStdString());
    todo.setPriority(priorityCombo->currentData().toInt());
...
void EditTodoDialog::showError(const QString& message) {
    errorLabel->setText(message);
    errorLabel->show();
}

void EditTodoDialog::clearError() {
    errorLabel->hide();
}

void EditTodoDialog::onCancel() {
